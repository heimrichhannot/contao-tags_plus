	...
	public function getTagList($blnExcludeUnpublishedItems = true)
	{
		if (count($this->arrCloudTags) == 0)
		{
			...
			else
			{
				...
				// HHFIX
                if ($this->tags_additionalWhereSql)
                {
                    if ($strCondPublished)
                    {
                        $strCondPublished .= ' AND ' . $this->tags_additionalWhereSql;
                    }
                    else
                    {
                        $strCondPublished .= 'WHERE ' . $this->tags_additionalWhereSql;
                    }
                }

                if ($this->tags_additionalSql)
                {
                    $tagtable .= ' ' . $this->tags_additionalSql;
                }
